 ...
 * @author Marek Drogon
 */
public class PceComputationFactory {

  private static final Logger logger = PceeLoggerFactory.getLogger(PceComputationFactory.class);
  ...
  public static String byteArrayToRawMessage(byte[] byteArray) {
    ...
    StringBuilder rawMessageString = new StringBuilder();

    for (byte b : byteArray) {
      rawMessageString.append(byteToBinaryStringConverter(b));
    }

    return rawMessageString.toString();
  }
  ...
  public static byte binaryStringToByteConverter2(String rawString) {
    ...
    for (int i = 0; i < 7; i++) {
      if (bitArray[i] == '1') x += 1 << i;
    }

    if (bitArray[7] == '1') x = x - 128;
  ...
  public static long getDecimalValue(String headerMember) {
    ...
    for (int i = 0; i < headerMember.length(); i++) {
      x +=
          Integer.parseInt(
                  headerMember.substring(headerMember.length() - 1 - i, headerMember.length() - i))
              * Math.pow(2, i);
    }
  ...
  public static String setDecimalValue(
      long decimalValue, long headerMemberMaxValue, int headerMemberLength) {
  ...
  public static String setBinaryString(
      String headerMember, int startingBit, String binaryString, int headerMemberLength) {

    if (startingBit >= headerMemberLength) {
  ...
  public static String checkInputBinaryString(String rawBinaryString, int headerMemberLength) {

    // TODO this should be an exception.
    String binaryString = rawBinaryString.replaceAll("[^01]", "0");
