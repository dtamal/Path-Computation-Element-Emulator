...
public class ConnectorGUI extends JFrame implements ActionListener {
  ...
  JCheckBox openMessagePFlagCheckBox,
      openMessageIFlagCheckBox,
      requestMessagePFlagCheckBox,
      requestMessageIFlagCheckBox,
      oFlagCheckBox,
      bFlagCheckBox,
      rFlagCheckBox,
      endPointsPFlag,
  ...
  private void buildConnectionPanel() {
    ...
    else serverPortTextField = new JTextField("4189");
  ...
  public void requestMessage() throws IOException {
    ...
    logger.info(
        "Requesting a Way from "
            + sourceAddress.getIPv4Address(true)
            + " to "
            + destinationAddress.getIPv4Address(true));
