...
public class PCEEServerControlResource {
  ...
  private ModuleManagement serverModuleManagement = PCEEWebLauncher.getServerModuleManagement();
  ...
  public Response startServer(String topology) {
    ...
    if (serverModuleManagement == null) {
      PCEEWebLauncher.setServerModuleManagement(new ModuleManagement(topology, true));
      return Response.ok()
          .header("Access-Control-Allow-Origin", "*")
          .header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
          .build();
  ...
  public Response getNodes() {
    ...
    if (this.topology.equals("austria")) scalingFactor = 1.7;

    List<String> setOfNodes = new ArrayList<String>();

    for (VertexElement v :
        TopologyInformation.getInstance().getGraph().createCopy().getVertexSet()) {
      setOfNodes.add(
          v.getVertexID()
              + " "
              + v.getXCoord() * scalingFactor
              + " "
              + v.getYCoord() * scalingFactor);
    }

    return Response.ok(setOfNodes)
        .header("Access-Control-Allow-Origin", "*")
        .header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
        .build();
  }
  ...
  public Response getLinks() {
    ...
    for (EdgeElement e : TopologyInformation.getInstance().getGraph().createCopy().getEdgeSet()) {
      setOfLinks.add(
          e.getSourceVertex().getVertexID() + " " + e.getDestinationVertex().getVertexID());
    }

    return Response.ok(setOfLinks)
        .header("Access-Control-Allow-Origin", "*")
        .header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
        .build();
  }
