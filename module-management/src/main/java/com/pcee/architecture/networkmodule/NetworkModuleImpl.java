...
public class NetworkModuleImpl extends NetworkModule {
  ...
  private static Logger logger = PceeLoggerFactory.getLogger(NetworkModuleImpl.class);
  ...
  // Selection Key
  private HashMap<String, SelectionKey> addressToSelectionKeyHashMap =
      new HashMap<String, SelectionKey>();

  // Map to store correlation between the session ID and the corresponding
  // socket channel
  private HashMap<String, SocketChannel> addressToSocketChannelHashMap =
  ...
  // socket layer
  private LinkedBlockingQueue<SocketChannel> registerConnQueue =
  ...
  public NetworkModuleImpl(boolean isServer, ModuleManagement layerManagement, int port) {
    logger.debug(
        "Entering: NetworkModuleImpl(boolean isServer, ModuleManagement layerManagement, int port)");
  ...
  public void registerConnection(
    ...
    // modules
    logger.info("Trying to initialise a Connection to " + address.getIPv4Address(false));
    try {
      // Opening a new connection to the remote peer
      SocketChannel socketChannel = SocketChannel.open();
      socketChannel.socket().setReuseAddress(true);
      socketChannel.connect(
          new InetSocketAddress(address.getIPv4Address(false), address.getPort()));
      ...
      if (socketChannel.isConnected() == true) {
        ...
        // message
        lm.getSessionModule().registerConnection(remoteAddress, true, true, forceClient);
      ...
      logger.error(
          "Could not connect to Server. Please Check if server is running on the remote address");
    } catch (IOException e) {
          ...
          public void run() {
            ...
            while (!selectorStop) {
              try {
                ...
                logger.info(
                    "Thread Interrupted when reading new connections from the register connection queue");
              }
            }
          }
        };
  ...
  /**
   * Function to initialize the selector, and if server, start a serversocketchannel to recieve
   * connections
   */
    ...
    try {
      ...
      if (isServer) {
        ...
        try {
          ...
          logger.error(
              "The PCEP Port is Already in use by another application. Terminating Server Instance");
          System.exit(-1);
        }
  ...
  private void connectionReceived(SocketChannel socketChannel) throws IOException {
    ...
    if (getSelectionKeyFromHashMap(address) == null) {
      ...
      logger.info(
          "Terminating incoming connection as a connection from the IP address"
    ...
    if (inputSocketChannel.isConnected()) {
      try {
        ...
        while (true) {
          ...
          if (byteCounter == -1) {
            logger.info(
                "Socket Shut Down Cleanly, Closing Connection for address: "
                    + address.getIPv4Address());
          ...
          if (byteCounter < -1) {
            // Unknown error
            logger.info(
                "Unknown error in socket. Closing Connection from address: "
                    + address.getIPv4Address());
            lm.getSessionModule().closeConnection(address);
            break;
          }
          ...
          if (byteCounter == 0) {
            ...
            if ((loopCount == 0) && (flag == 0)) {
              ...
              // Closing Connection
              logger.info(
                  "Selector in read loop with no data to read, Closing Connection from address: "
                      + address.getIPv4Address());
              lm.getSessionModule().closeConnection(address);
              break;
            }
        ...
        logger.info(
            "Error when reading from socket for address "
                + address.getIPv4Address()
                + " Closing connection");
        lm.getSessionModule().closeConnection(address);
      }
      ...
      logger.debug(
          "| Input Channel Closed for " + address.getIPv4Address() + " Closing connection");
      lm.getSessionModule().closeConnection(address);
    }
  }

  /**
   * Function to check if the incoming string has multiple concatenated messages if one of the
   * messages is smaller than a full message, adds this message to the insertPartialMessageToHashMap
   * based buffer
   *
   * @param binaryString
   * @param address
