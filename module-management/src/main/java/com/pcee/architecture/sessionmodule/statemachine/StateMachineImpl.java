...
public abstract class StateMachineImpl extends StateMachine {
  ...
  public StateMachineImpl(
    ...
    this.enterIdleState();
    if (connectionInitialized) this.updateState(true);
  }
  ...
  public synchronized void updateState(boolean connectionEstablished) {
    ...
    if (state != 1) {
      logger.info(
          "Received connectionEstablished Update in the wrong state! Should have been state 1, received it in state{}",
          state);
    }
  ...
  protected void enterOpenWaitState(PCEPMessage message) {
    ...
    if (messageType != 1) { // (h)
      logger.info("Received wrong Message Type. Should have been type 1. Got: {}", messageType);
    ...
    if (noErrorsDetected && sessionCharacteristics == 0) { // (c)
      logger.info(
          "No Error detected, but Session Characteristics are not acceptable, but negotiable");
    ...
    if (noErrorsDetected && sessionCharacteristics == -1) { // (e)
      logger.info(
          "No Error detected, but Session Characteristics are not acceptable, and not negotiable");
    ...
    if (noErrorsDetected && openRetry == 0 && sessionCharacteristics == 0) { // (f)
      logger.info(
          "No Error detected, openRetry==0, and Session Characteristics are not acceptable, but negotiable");
  ...
  protected void enterKeepWaitState(PCEPMessage message) {
    ...
    if (messageType != 2 && messageType != 6) { // (e)
      logger.info("Received wrong Message Type. Should have been type 2,6. Got: {}", messageType);
      this.releaseResources();
    }
    ...
    if (messageType == 6) {
      ...
      logger.info("remoteOk is {}", remoteOk);
      if (remoteOk) {
        this.restartKeepWaitTimer();
        this.setState(3);
      }
      ...
      case 2:
        {
          ...
          restartDeadTimer();
          logger.info("Processing Information: {}", message.contentInformation());
      ...
      case 3:
        {
          ...
          } else lm.getSessionModule().sendMessage(message, ModuleEnum.COMPUTATION_MODULE);
  ...
  protected void terminateTimerTasks() {
    ...
    if (connectTimerTask != null) cancelConnectTimer();
    if (openWaitTimerTask != null) cancelOpenWaitTimer();
    if (keepWaitTimerTask != null) cancelKeepWaitTimer();
    if (deadTimerTask != null) cancelDeadTimer();
    if (keepAliveTimerTask != null) cancelKeepAliveTimer();
  }
  ...
  protected void restartDeadTimer() {
    ...
    } else this.deadTimerTask.cancel();
    deadTimerRunning = true;
    deadTimerTask =
        new TimerTask() {
          public void run() {
