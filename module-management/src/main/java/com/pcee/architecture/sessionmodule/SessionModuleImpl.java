...
public class SessionModuleImpl extends SessionModule {
  ...
  public SessionModuleImpl(ModuleManagement layerManagement, int sessionThreads) {
    logger.debug(
        "Entering: SessionModuleImpl(ModuleManagement layerManagement, int sessionThreads)");
  ...
  private class ReadingQueueBuffer {
    ...
    public ReadingQueueBuffer(PceMessage message, ModuleEnum sourceLayer) {
  ...
  private class ReadingQueueThreadImpl extends Thread {
    ...
    LinkedBlockingQueue<ReadingQueueBuffer> readingQueue =
        new LinkedBlockingQueue<ReadingQueueBuffer>();
     ...
     * @param message
     */
    public void addMessage(PceMessage message, ModuleEnum sourceLayer) {
      logger.debug("Entering: addMessage(PceMessage message)");
  ...
  public void stop(boolean graceful) {
    ...
    if (graceful) {
      ...
      for (int i = 0; i < sessionThreads; i++) readingQueueThread[i].interrupt();
  ...
  public void receiveMessage(PceMessage message, ModuleEnum sourceLayer) {
    ...
    switch (sourceLayer) {
      case NETWORK_MODULE:
        ...
        // TODO
        x = Integer.parseInt(message.getAddress().getIPv4Address().split(":")[1]) % sessionThreads;
      ...
      case CLIENT_MODULE:
        ...
        break;
      default:
        logger.info("Error in recieveMessage(PceMessage message, LayerEnum sourceLayer)");
  ...
  public void registerConnection(
    ...
    if (!addressToStateMachineHashMap.containsKey(address.getIPv4Address())) {
      ...
      if ((connectionInitialized == true) && (connected == false)) {
        lm.getNetworkModule()
            .registerConnection(address, connected, connectionInitialized, forceClient);
      }
  ...
  public void closeConnection(PceAddress address) {
    logger.debug("Entering: closeConnection(PceAddress address)");
    logger.debug("| address: " + address.getIPv4Address());
    ...
    if (addressToStateMachineHashMap.containsKey(address.getIPv4Address())) {
      ...
      logger.debug("Could not find a StateMachine for " + address.getIPv4Address());
    }
  }
  ...
  private void createNewStateMachine(
      PceAddress address, boolean connectionInitialized, boolean forceClient) {
    logger.debug(
        "Entering: createNewStateMachine(PceAddress address, boolean connectionInitialized)");
    logger.debug("| address: " + address.getIPv4Address());
    logger.debug("| connectionInitialized: " + connectionInitialized);

    logger.info("New StateMachine for " + address.getIPv4Address());
    // Creating new state machine
    // If LM is of type client, create a client state machine or else create a server state machine
    // (by default)
    StateMachine stateMachine;
    if (lm.isServer()) {
      // If in the server a state machine is registered forcefully as a client then we create a
      // client state machine or else we make
      // a server state machine
      if (forceClient)
        stateMachine =
            new StateMachineClientImpl(lm, address, stateMachineTimer, connectionInitialized);
      else
        stateMachine =
            new StateMachineServerImpl(lm, address, stateMachineTimer, connectionInitialized);
    } else {
      stateMachine =
          new StateMachineClientImpl(lm, address, stateMachineTimer, connectionInitialized);
    }
   ...
   * @param message
   */
  private void processMessage(PceMessage message, ModuleEnum sourceLayer) {
    logger.debug("Entering: processMessage(PceMessage message)");
    logger.debug("| message: " + message.contentInformation());
    logger.debug("| address: " + message.getAddress().getIPv4Address());

    logger.info("Processing Message from " + message.getAddress().getIPv4Address());
    StateMachine machine = getStateMachineFromHashMap(message.getAddress());
    if (machine == null) {
      logger.info(
          "State Machine for connection from "
              + message.getAddress().getIPv4Address()
              + " does not exist. Discarding Message");
    } else machine.updateState(message, sourceLayer);
  }
   ...
   * @return
   */
  private StateMachine getStateMachineFromHashMap(PceAddress address) {
    logger.debug("Entering: getStateMachineFromHashMap(PceAddress address)");
    logger.debug("| address: " + address.getIPv4Address());
   ...
   * @param stateMachine
   */
  private void insertStateMachineToHashMap(PceAddress address, StateMachine stateMachine) {
    logger.debug(
        "Entering: insertStateMachineToHashMap(Address address, StateMachineImpl stateMachine)");
    logger.debug("| address: " + address.getIPv4Address());
    logger.debug("| stateMachine: " + stateMachine.toString());

    logger.info("Inserting StateMachine for " + address.getIPv4Address());
    addressToStateMachineHashMap.put(address.getIPv4Address(), stateMachine);
    logger.info("| StateMachines active: " + addressToStateMachineHashMap.size());
  }
   ...
   * @param address
   */
  private void removeStateMachineFromHashMap(PceAddress address) {
    logger.debug("Entering: removeStateMachineFromHashMap(PceAddress address)");
    logger.debug("| address: " + address.getIPv4Address());
