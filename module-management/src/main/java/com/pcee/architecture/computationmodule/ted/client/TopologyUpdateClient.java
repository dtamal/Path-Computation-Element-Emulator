...
public class TopologyUpdateClient {
  ...
  public static void reserveCapacity(
      String ip, int port, double capacity, ArrayList<String> vertexSequence) {
    Map map = new HashMap();
  ...
  @SuppressWarnings({"rawtypes", "unchecked"})
  public static void updateEdgeDefinition(
      String ip,
      int port,
      String sourceID,
      String destID,
  ...
  @SuppressWarnings({"rawtypes", "unchecked"})
  public static void releaseCapacity(
      String ip, int port, double capacity, ArrayList<String> vertexSequence) {
    Map map = new HashMap();
  ...
  public static void main(String[] args) {
    ...
    Constraint constr =
        new SimplePathComputationConstraint(graph.getVertex(sourceID), graph.getVertex(destID), 10);

    PathElement temp = algo.computePath(graph, constr);

    if (temp != null) {
      //			EdgeElement edge = new EdgeElement (sourceID + "-" + destID, graph.getVertex(sourceID),
      // graph.getVertex(destID), graph);
      ArrayList<String> vertexSequence = new ArrayList<String>();
      ArrayList<VertexElement> vertices = temp.getTraversedVertices();
      for (int i = 0; i < vertices.size(); i++) vertexSequence.add(vertices.get(i).getVertexID());
