...
public class TopologyInformation {
  ...
  private static Logger logger = PceeLoggerFactory.getLogger(TopologyInformation.class);
  ...
  private static TopologyInformation _instance;
  ...
  /**
   * @param importer ONLY SNDLib or BRITE supported as input
   */
  ...
  private TopologyInformation() {
    ...
    if (graph == null) logger.debug("Error in loading graph from file");
    else logger.info("NetworkSize: {}", networkSize());
  ...
  public static TopologyInformation getInstance() {
    if (_instance == null) _instance = new TopologyInformation();
    return _instance;
  }
    ...
    topologyUpdateThread =
        new Thread() {
          ...
          public String parseInput(String text) {
                ...
                if (input.get("operation").toString().equalsIgnoreCase("reserve")) {
                  ...
                  synchronized (graph) {
                    ...
                    for (i = 0; i < vertexSequence.size() - 1; i++) {
                      ...
                      if (graph.aConnectingEdge(sourceID, destID)) {
                        if (!graph
                            .getConnectingEdge(sourceID, destID)
                            .getEdgeParams()
                            .reserveCapacity(capacity)) {
                          logger.info(
                              "Cannot reserve capacity between {} and {}", sourceID, destID);
                          for (int j = 0; j < i; j++) {
                        ...
                        logger.info(
                            "Invalid Vertex Sequence sent, no edge found between {} and {}",
                            sourceID,
                            destID);
                        for (int j = 0; j < i; j++) {
                          // Releasing capacity that was reserved till before i
                    ...
                    for (i = 0; i < vertexSequence.size() - 1; i++) {
                      ...
                      if (graph.aConnectingEdge(sourceID, destID)) {
                        if (!graph
                            ...
                            .releaseCapacity(capacity)) {
                          logger.info(
                              "Cannot release additional capacity between {} and {}",
                              sourceID,
                              destID);
                          for (int j = 0; j < i; j++) {
                            // Releasing capacity that was reserved till before i
                  ...
                  synchronized (graph) {
                    ...
                    if (graph.aConnectingEdge(sourceID, destID)) {
                      ...
                      logger.info("Updated Edge definition from {} to {}", sourceID, destID);
                      Map map = new HashMap();
                      map.put("response", true);
                      return json.toJson(map);
                    } else {
                      logger.info(
                          "No existing edge from {} to {} foud in topology", sourceID, destID);
                      Map map = new HashMap();
                      map.put("response", false);
                      map.put(
                          "reason",
                          "No existing edge from "
                              + sourceID
                              + " to "
                              + destID
                              + " foud in topology");
            ...
            Map map = new HashMap();
            map.put("response", false);
            map.put("reason", "Invalid or unknown operation");
            return json.toJson(map);
          }

          // Override the run() method to implement a simple server socket to listen for topology
          // updates
          public void run() {
            ...
            try {
              ...
              logger.debug(
                  "Could not open server socket to listen for topology updates on port:{}",
                  topologyUpdatePort);
            }
          }
        };
