...
public class Worker extends Thread {
	 ...
	 * @param requestQueue
	 */
	public Worker(ModuleManagement layerManagement, ThreadPool pool, String ID, LinkedBlockingQueue<PCEPMessage> requestQueue){
		lm = layerManagement;
		this.pool = pool;
		this.ID = ID;
...
//					logger.info("Starting request ID " + request.getRequestID());
			...
			flag=1;
			if (request!=null){
				task = new WorkerTask(lm, request, TopoManager.get_instance().getManager());
				task.run();
