...
public class PathParams {
  ...
  public double getPathDelay() {
    ...
    if (validateEdgeIterator(iter) == false) return -1;

    while (iter.hasNext() == true) delay += iter.next().getEdgeParams().getDelay();
    return delay;
  }
  ...
  public double getPathWeight() {
    ...
    while (iter.hasNext() == true) cost += iter.next().getEdgeParams().getWeight();
    return cost;
  }
