...
public class SimplePathComputationAlgorithm implements PathComputationAlgorithm {
  ...
  protected ArrayList<PathElement> sortPaths(ArrayList<PathElement> paths) {
    ...
    while (flag == 0) {
      ...
      for (int i = paths.size() - 1; i > count; i--)
        if (paths.get(i).getPathParams().getPathWeight()
            < paths.get(i - 1).getPathParams().getPathWeight()) {
  ...
  public PathElement computePath(Gcontroller graph, Constraint constr) {
    ...
    if (source.getConnectedEdges() == null) return null;
      ...
      if (checkConstraint(constraint, edge) == 1) {
        ...
        if (edge.getSourceVertex().compareTo(source) == 0) {
          tmp =
              new PathElementImpl(
                  graph,
                  graph.getVertex(constraint.getSource().getVertexID()),
    ...
    while (list.size() > 0) {
      ...
      if (visitedVertices.contains(temp.getDestination()) == false) {
        ...
        // if
        // (destination.compareTo(graph.getVertex(constraint.getDestination().getVertexID()))==0){
        ...
        if (destination.getConnectedEdges() != null) {
          ...
          while (iter.hasNext()) {
            ...
            } else nextDestination = edge.getSourceVertex();
            ...
            if (visitedVertices.contains(nextDestination) == false) {
              if (temp.containsVertex(nextDestination) == false) {
                //								System.out.println("Size of Edge list for new Path Element = " +
                // temp.getTraversedEdges().size());
                // Check with constraint if edge can be added
                if (checkConstraint(constraint, edge, temp) == 1) {
                  tmp =
                      new PathElementImpl(
                          graph, constraint.getSource(), nextDestination, temp.getTraversedEdges());
                  tmp.insertEdge(edge);
                  list.add(tmp);
                }
              }
    ...
    //		GraphLogger.logError("No Path found from " + source.getVertexID() + " to " +
    // constraint.getDestination().getVertexID(), classIdentifier);
    GraphLogger.logError(
        "No Path found from "
            + constraint.getSource().getVertexID()
            + " to "
            + constraint.getDestination().getVertexID(),
        classIdentifier);
    return null;
  }
}
