...
public class StaticPathSortImpl {
  ...
  /**
   * Sort paths by ascending order of available capacity. In case of tie give preference to shortest
   * weight path
   */
  public static ArrayList<PathElement> sortPathsByBandwidth(ArrayList<PathElement> paths) {
    ...
    while (flag == 0) {
      flag = 1;
      for (int i = paths.size() - 1; i > count; i--)
        if (paths.get(i).getPathParams().getAvailableCapacity()
            > paths.get(i - 1).getPathParams().getAvailableCapacity()) {
          // swap elements i and i-1
          PathElement temp = paths.remove(i - 1);
          paths.add(i, temp);
          flag = 0;
        } else if ((paths.get(i).getPathParams().getAvailableCapacity()
                == paths.get(i - 1).getPathParams().getAvailableCapacity())
            && (paths.get(i).getPathParams().getPathWeight()
                < paths.get(i - 1).getPathParams().getPathWeight())) {
          // swap elements i and i-1
          PathElement temp = paths.remove(i - 1);
          paths.add(i, temp);
  ...
  public static ArrayList<PathElement> sortPathsByDelay(ArrayList<PathElement> paths) {
    ...
    while (flag == 0) {
      ...
      for (i = paths.size() - 1; i > count; i--) {
        if (paths.get(i).getPathParams().getPathDelay()
            < paths.get(i - 1).getPathParams().getPathDelay()) {
          // swap elements i and i-1
          PathElement temp = paths.remove(i - 1);
          paths.add(i, temp);
          flag = 0;
