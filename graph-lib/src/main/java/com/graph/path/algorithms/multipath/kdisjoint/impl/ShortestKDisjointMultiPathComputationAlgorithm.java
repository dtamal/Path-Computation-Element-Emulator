...
public class ShortestKDisjointMultiPathComputationAlgorithm
    implements MultiPathComputationAlgorithm {
        ...
        for (int i = 0; i < edges1.size(); i++) {
          ...
          if (flag1 == true) {
            ...
            for (j = 1; j < edges1.size() - i; j++) {
              if (!oldPath.containsEdge(edges1.get(j + i))) break;
            }

            // j indicates the number of common edges starting from i that are common between
            // oldPath and temp1
            d1 = i + j - 1;
            ...
            for (j = 0; j < s0; j++) edgeSequence.add(edges0.get(j));
            for (j = d1 + 1; j < edges1.size(); j++) edgeSequence.add(edges1.get(j));
            oldPath.setEdgeSequence(edgeSequence);

            // tempPath
            edgeSequence = new ArrayList<EdgeElement>();
            for (j = 0; j < s1; j++) edgeSequence.add(edges1.get(j));
            for (j = d0 + 1; j < edges0.size(); j++) edgeSequence.add(edges0.get(j));
  ...
  public void refreshEdgeElimination(Gcontroller controller, ArrayList<PathElement> computedPaths) {
    ...
    while (iter1.hasNext()) {
      ...
      if (params.getWeight() < 0) params.setWeight(-1.0 * params.getWeight());
    }
  ...
  private ArrayList<PathElement> changeParentGraph(
      Gcontroller graph, ArrayList<PathElement> paths) {
    ...
    for (int i = 0; i < paths.size(); i++) {
      ...
      newPaths.add(
          new PathElementImpl(
              graph,
              graph.getVertex(paths.get(i).getSourceID()),
              graph.getVertex(paths.get(i).getDestinationID()),
              newEdges));
    }
    return newPaths;
  }

  public ArrayList<PathElement> computePath(
      Gcontroller controller, MultiPathConstraint constraints) {
  ...
  public static void main(String[] args) {
    ...
    ShortestKDisjointMultiPathComputationAlgorithm algo =
        new ShortestKDisjointMultiPathComputationAlgorithm();
    MultiPathConstraint constr =
        new SimpleMultiPathComputationConstraint(graph.getVertex("1"), graph.getVertex("8"), 2, 10);
