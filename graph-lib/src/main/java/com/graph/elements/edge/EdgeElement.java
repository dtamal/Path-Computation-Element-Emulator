...
public class EdgeElement {
  ...
  /** default Constructor */
  public EdgeElement(
      String edgeID, VertexElement source, VertexElement destination, Gcontroller graph) {
    this.edgeID = edgeID;
    this.source = source;
    this.destination = destination;
    this.graph = graph;
  ...
  public VertexElement getSourceVertex() {
    if (source != null) return this.source;
    else {
  ...
  public VertexElement getDestinationVertex() {
    if (destination != null) return this.destination;
    else {
  ...
  public EdgeElement copyEdgeElement(Gcontroller newGraph) {
    EdgeElement element =
        new EdgeElement(
            edgeID,
            newGraph.getVertex(source.getVertexID()),
            newGraph.getVertex(destination.getVertexID()),
            newGraph);
