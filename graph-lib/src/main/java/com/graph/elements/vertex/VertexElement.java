...
public class VertexElement implements Comparable<VertexElement> {
  ...
  public void updateNeighbourMap() {
    ...
    while (iter.hasNext()) {
      ...
      if (this.graph.aConnectingEdge(this.vertexID, remoteNodeID) == true) {
        this.neighbourMap.put(
            this.graph.getVertex(remoteNodeID),
            this.graph.allConnectingEdges(this.vertexID, remoteNodeID));
      }
    }
  }
  ...
  public Set<VertexElement> getNeighbouringVertices() {
    if (validateNeighbourMap()) return neighbourMap.keySet();
    else return null;
  }
  ...
  public void insertConnectedEdge(EdgeElement edge) {
    ...
    if (edge.graph != this.graph) {
      GraphLogger.logError(
          "The edge to be inserted does not belong to the same graph", classIdentifier);
    } else {
      ...
      if (edge.getSourceVertex() == this) {
        ...
        GraphLogger.logError(
            "The edge to be inserted is not connected to the vertex " + this.vertexID,
            classIdentifier);
      }
      if (destination != null) {
  ...
  public int compareTo(VertexElement temp) {
    if ((temp.getVertexID().compareTo(this.vertexID) == 0)) return 0;
    return 1;
  }
  ...
  public VertexElement copyVertexElement(Gcontroller newGraph) {
    ...
    if (this.getVertexParams() != null) params = this.getVertexParams().copyVertexParams(element);
  ...
  public void excludeEdge(EdgeElement edge) {
    if (this.getConnectedEdges().contains(edge)) this.excludedEdges.add(edge);
  }
}
