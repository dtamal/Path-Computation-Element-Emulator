...
public class JDSLGraphImpl extends IncidenceListGraph {
  ...
  public void addVertex(VertexElement vertex) {
    ...
    } else vertexMap.put(vertex.getVertexID(), temp);
  }
  ...
  public VertexElement getVertexElement(String vertexID) {
    ...
    if (vertex != null) return ((VertexElement) vertex.element());
    else {
      GraphLogger.logError(
          "Vertex with ID :" + vertexID + " not found in vertexMap.", classIdentifier);
      return null;
    }
  }
  ...
  public EdgeElement getEdgeElement(String edgeID) {
    ...
    if (edge != null) return ((EdgeElement) edge.element());
    else {
  ...
  public boolean addEdge(EdgeElement edge) {
    Edge temp =
        this.insertEdge(
            getVertex(edge.getSourceVertex().getVertexID()),
            getVertex(edge.getDestinationVertex().getVertexID()),
            edge);
    if (temp == null) {
  ...
  public EdgeElement getConnectingEdge(String sourceID, String destinationID) {
    ...
    if (edge == null) {
      GraphLogger.logError(
          "No Edge connecting " + sourceID + " to " + destinationID + " in the JDSL Graph",
          classIdentifier);
      return null;
    }
